...
static u32 bailout_threhold = 2;
static u8 *hashhit;
static float nonewhashrate;
static u32 seedcount = 0;
static u32 skipstep = 0x10;
...
void dumpqueue(){
    struct queue_entry *ptrptr = queue;
    int i;
    loginfo("queue: ");
    i = 0;
    while(ptrptr){
        loginfo("\t%5d : fuzzed = %d facored = %d depth = %lld %s",
                i,
                ptrptr->was_fuzzed,
                ptrptr->favored,
                ptrptr->depth,
...
static void add_to_queue(u8 *fname, u32 len, u8 passed_det, u8 aIsSeed, u32 offset) {
...
static void read_testcases(void) {
    ...
    for (i = 0; i < nl_cnt; i++) {
        ...
        add_to_queue(fn, st.st_size, passed_det, 1, 0xffffffff);
        seedcount++;
    }
...
static u8 calibrate_case(char **argv, struct queue_entry *q, u8 *use_mem,
    ...
    for (stage_cur = 0; stage_cur < stage_max; stage_cur++) {
        ...
        u32 curr_hash = hash32(trace_bits, MAP_SIZE, HASH_CONST);
        hashhit[curr_hash>>3] |= 1 << (curr_hash&7);
...
static u8 save_if_interesting(char **argv, void *mem, u32 len, u8 fault) {
    ...
    if (fault == crash_mode) {
        ...
        add_to_queue(fn, len, 0, 0, stage_cur_byte);
...
static void show_stats(void) {
    ...
    sprintf(tmp + banner_pad, "%s " cLCY VERSION cLGN " (%s)",
            crash_mode ? cPIN "peruvian were-rabbit" : cYEL "AFL-BSO",
            use_banner);
...
#define SP20 SP10 SP10
    ...
    SAYF(bV bSTOP "        trim : " cRST "%-37s " bSTG bVR bH20 bH2 bH2 bRB
            "\n" bLB bH30 bH20 bH2 bH bRB bSTOP cRST ,
            tmp);
    ...
    if (cpu_core_count) {
        ...
        SAYF("\r");
    SAYF("\n" bSTG bLT bH20 bH2 bSTOP cCYA " other info " cRST bSTG bH20 bH20 bH2 bH2 bRT);
    SAYF("\n" bSTG bV bSTOP "   curr name : " cRST "%-62s " bSTG bV, queue_cur->fname);
    SAYF("\n" bSTG bV bSTOP "         len : " cRST "%-62s " bSTG bV, DI(queue_cur->len));
    SAYF("\n" bSTG bV bSTOP "     exec_us : " cRST "%-62lld " bSTG bV, queue_cur->exec_us);
    SAYF("\n" bSTG bV bSTOP "     favored : " cRST "%-62d " bSTG bV, queue_cur->favored);
    SAYF("\n" bSTG bV bSTOP "  was_fuzzed : " cRST "%-62d " bSTG bV, queue_cur->was_fuzzed);
    SAYF("\n" bSTG bV bSTOP "       depth : " cRST "%-62lld " bSTG bV, queue_cur->depth);
    SAYF("\n" bSTG bV bSTOP "  b-threhold : " cRST "%-62u " bSTG bV, bailout_threhold);
    SAYF("\n" bSTG bV bSTOP "       skiped: " cRST "%-62s " bSTG bV, DI(skiped_count));
    SAYF("\n" bSTG bV bSTOP "old zone rate: " cRST "%-62f " bSTG bV, nonewhashrate);
    SAYF("\n" bSTG bV bSTOP "    seedcount: " cRST "%-62u " bSTG bV, seedcount);
    SAYF("\n" bSTG bV bSTOP "       offset: " cRST "%-62x " bSTG bV, queue_cur->offset);
    SAYF("\n" bSTG bLB bH30 bH20 bH20 bH5 bH2 bH bRB);
    SAYF(RESET_G1);
...
EXP_ST u32 run_and_get_hash(char **argv, u8 *out_buf, u32 len) {
...
static u8 fuzz_one(char **argv) {
    ...
    u32 singlestageruncount = 0;
    u8* hashnochangemap = NULL;
    u64 startfuzztime = get_cur_time();

    u8 ret_val = 1, doing_det = 0;

    u8 a_collect[MAX_AUTO_EXTRA];
    u32 a_len = 0;
    u32 fbyte8 = 0, fbyte16 = 0, fbyte32 = 0;

    loginfo("current_entry: %d", current_entry);
    loginfo("fname: %s", queue_cur->fname);
    loginfo("cal_failed = %d", queue_cur->cal_failed);
    loginfo("trim_done = %d", queue_cur->trim_done);
    loginfo("was_fuzzed = %d", queue_cur->was_fuzzed);
    loginfo("passed_det = %d", queue_cur->passed_det);
    loginfo("has_new_cov = %d", queue_cur->has_new_cov);
    loginfo("var_behavior = %d", queue_cur->var_behavior);
    loginfo("favored = %d", queue_cur->favored);
    loginfo("fs_redundant = %d", queue_cur->fs_redundant);
    loginfo("bitmap_size = %d", queue_cur->bitmap_size);
    loginfo("exec_cksum = %08x", queue_cur->exec_cksum);
    loginfo("exec_us = %lld", queue_cur->exec_us);
    loginfo("handicap = %lld", queue_cur->handicap);
    loginfo("depth = %lld", queue_cur->depth);
    ...
    loginfo("Map the test case into memory.");
    ...
    loginfo("md5 = %s", md5sumhex(in_buf, len));

    /*********************
     * PERFORMANCE SCORE *
     *********************/
    loginfo("performance score");
    ...
    stage_name = "eval128";
    stage_short = "eval128";
    stage_max = len / skipstep;

    orig_hit_cnt = new_hit_cnt;

    u32 nonewhashcount = 0;

    if( len > skipstep){
        hashnochangemap = ck_alloc(len/skipstep);

        for (i = 0; i < len - skipstep; i+=skipstep) {
            stage_cur_byte = i;
            stage_cur = i/skipstep;


            int j;
            for(j=0; j<skipstep; j++){
                *(u8*)(out_buf + i + j) ^= 0xFF;
            }

            u32 curr_hash = run_and_get_hash(argv, out_buf, len);
            if( (hashhit[curr_hash>>3] >> (curr_hash & 7)) & 1 ){
                hashnochangemap[i/skipstep] = 1;
                nonewhashcount++;
            }
    ...
    nonewhashrate = nonewhashcount*1.0 / len * skipstep;


    /*********************************************
     * calc low bound and high bound             *
     *********************************************/

    u32 lowbound = 0;
    u32 highbound = len;
    if( len > 0x1000 ){
        if( queue_cur->offset > 0x100){
            lowbound = queue_cur->offset - 0x100;
        }
        if( ( queue_cur->offset + 0x100 ) < len ){
            highbound = queue_cur->offset + 0x100;
        }
    }
...
#define EFF_SPAN_ALEN(_p, _l) (EFF_APOS((_p) + (_l)-1) - EFF_APOS(_p) + 1)
    ...
    singlestageruncount = 0;

    for (stage_cur = 0; stage_cur < stage_max; stage_cur++) {

        stage_cur_byte = stage_cur;

        if( hashnochangemap && hashnochangemap[stage_cur_byte / skipstep] ){
            skiped_count++;
            continue;
        }
        if( queue_cur->offset != 0xffffffff &&          // fuzz all for initial seeds
                stage_cur_byte > 0x100){                // always fuzz first 256 bytes
            if( lowbound > stage_cur_byte || stage_cur_byte > highbound ){
                continue;
            }
        }
        ...
        out_buf[stage_cur] ^= 0xFF;
        if( stage_cur % 500 == 0 && (( get_cur_time() - startfuzztime) > 5*60*1000) ){
            goto havoc_stage;
        }
    }
    ...
    fbyte8 = new_hit_cnt - orig_hit_cnt;

    stage_finds[STAGE_FLIP8] += new_hit_cnt - orig_hit_cnt;
    stage_cycles[STAGE_FLIP8] += singlestageruncount;
    ...
    for (i = 0; i < len - 1; i++) {
        ...
        if( hashnochangemap && 
                (hashnochangemap[stage_cur_byte / skipstep] || hashnochangemap[(stage_cur_byte+1) / skipstep])
          ){
            skiped_count++;
            continue;
        }
    ...
    new_hit_cnt = queued_paths + unique_crashes;
    fbyte16 = new_hit_cnt - orig_hit_cnt;

    stage_finds[STAGE_FLIP16] += new_hit_cnt - orig_hit_cnt;
    stage_cycles[STAGE_FLIP16] += singlestageruncount;
    ...
    for (i = 0; i < len - 3; i++) {
        ...
        if( hashnochangemap && 
                (hashnochangemap[stage_cur_byte / skipstep] || hashnochangemap[(stage_cur_byte+3) / skipstep])
          ){
            skiped_count++;
            continue;
        }
    ...
    new_hit_cnt = queued_paths + unique_crashes;
    fbyte32 = new_hit_cnt - orig_hit_cnt;

    stage_finds[STAGE_FLIP32] += new_hit_cnt - orig_hit_cnt;
    stage_cycles[STAGE_FLIP32] += singlestageruncount;

    if( (fbyte8 + fbyte16 + fbyte32) <= bailout_threhold ){
        goto havoc_stage;
    }
...
#define FLIP_BIT(_ar, _b)                                                      \
    ...
    stage_cycles[STAGE_FLIP1] += singlestageruncount;
    ...
    for (stage_cur = 0; stage_cur < stage_max; stage_cur++) {
        stage_cur_byte = stage_cur >> 3;
        if( hashnochangemap && 
                (hashnochangemap[stage_cur_byte / skipstep] || hashnochangemap[(stage_cur+1) / skipstep] )
          ){
            skiped_count++;
            continue;
        }
    ...
    stage_cycles[STAGE_FLIP2] += singlestageruncount;
    ...
    for (stage_cur = 0; stage_cur < stage_max; stage_cur++) {
        ...
        if( hashnochangemap && 
                (hashnochangemap[stage_cur_byte / skipstep] || hashnochangemap[(stage_cur+3) / skipstep] )
          ){
            skiped_count++;
            continue;
        }
    ...
    stage_cycles[STAGE_FLIP4] += singlestageruncount;
...
skip_bitflip:
    ...
    stage_cycles[STAGE_ARITH8] += singlestageruncount;
    ...
    stage_cycles[STAGE_ARITH16] += singlestageruncount;
    ...
    stage_cycles[STAGE_ARITH32] += singlestageruncount;
...
int main(int argc, char **argv) {
    ...
    while ((opt = getopt(argc, argv, "+i:o:f:m:t:T:dnCB:S:M:x:Qb:")) > 0)
        ...
        switch (opt) {
            ...
            case 'b': /* bailout threshold */

                if (in_dir)
                    FATAL("Multiple -i options not supported");
                bailout_threhold = atoi(optarg);
                if( bailout_threhold > 100 ){
                    FATAL("threhold value is too big");
                }
    ...
    hashhit = mmap(0, 0x100000000/8, 7, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
    printf("%p\n", hashhit);
    ...
    while (1) {
        ...
        loginfo("start fuzzone");
        skipped_fuzz = fuzz_one(use_argv);
        loginfo("fuzzone done");
