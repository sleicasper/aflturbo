...
static void show_stats(void) {
    ...
    sprintf(tmp + banner_pad, "%s " cLCY VERSION cLGN " (%s)",
            crash_mode ? cPIN "peruvian were-rabbit" : cYEL "AFL-bailout",
            use_banner);
...
static u8 fuzz_one(char **argv) {
    ...
    u32 fbit1 = 0, fbit2 = 0, fbit4 = 0;
    u32 arith8 = 0, arith16 = 0, arith32 = 0;
    u32 inter8 = 0, inter16 = 0, inter32 = 0;
    u32 extrauo = 0, extraui = 0, extraao = 0;
    u32 havoc = 0, splice = 0;
...
#define EFF_SPAN_ALEN(_p, _l) (EFF_APOS((_p) + (_l)-1) - EFF_APOS(_p) + 1)
    ...
    for (stage_cur = 0; stage_cur < stage_max; stage_cur++) {
        ...
        out_buf[stage_cur] ^= 0xFF;
        if( stage_cur % 500 == 0 && (( get_cur_time() - startfuzztime) > 4*60*1000) ){
            goto havoc_stage;
        }
    }
...
#define FLIP_BIT(_ar, _b)                                                      \
    ...
    fbit1 = new_hit_cnt - orig_hit_cnt;
    ...
    fbit2 = new_hit_cnt - orig_hit_cnt;
    ...
    fbit4 = new_hit_cnt - orig_hit_cnt;
...
skip_bitflip:
    ...
    arith8 = new_hit_cnt - orig_hit_cnt;
    ...
    arith16 = new_hit_cnt - orig_hit_cnt;
    ...
    arith32 = new_hit_cnt - orig_hit_cnt;
...
skip_arith:
    ...
    inter8 = new_hit_cnt - orig_hit_cnt;
    ...
    inter16 = new_hit_cnt - orig_hit_cnt;
    ...
    inter32 = new_hit_cnt - orig_hit_cnt;
...
skip_interest:
    ...
    extrauo = new_hit_cnt - orig_hit_cnt;
    ...
    extraui = new_hit_cnt - orig_hit_cnt;
...
skip_user_extras:
    ...
    extraao = new_hit_cnt - orig_hit_cnt;
...
havoc_stage:
    ...
    if (!splice_cycle) {
        havoc = new_hit_cnt - orig_hit_cnt;
        stage_finds[STAGE_HAVOC] += new_hit_cnt - orig_hit_cnt;
        stage_cycles[STAGE_HAVOC] += stage_max;
    } else {
        splice = new_hit_cnt - orig_hit_cnt;
...
retry_splicing:
    ...
    ret_val = 0;
    logdebug("fname: %s", queue_cur->fname);
    logdebug("\t fbyte: %d %d %d", fbyte8, fbyte16, fbyte32);
    logdebug("\t fbit: %d %d %d", fbit1, fbit2, fbit4);
    logdebug("\t arith: %d %d %d", arith8, arith16, arith32);
    logdebug("\t inter: %d %d %d", inter8, inter16, inter32);
    logdebug("\t dict: %d %d %d", extrauo, extraui, extraao);
    logdebug("\t havoc: %d %d", havoc, splice);
