...
static void add_to_queue(u8 *fname, u32 len, u8 passed_det, u32 offset) {
...
static void read_testcases(void) {
    ...
    for (i = 0; i < nl_cnt; i++) {
        ...
        add_to_queue(fn, st.st_size, passed_det, 0xffffffff);
    }
...
static u8 save_if_interesting(char **argv, void *mem, u32 len, u8 fault) {
    ...
    if (fault == crash_mode) {
        ...
        add_to_queue(fn, len, 0, stage_cur_byte);
...
static void show_stats(void) {
    ...
    sprintf(tmp + banner_pad, "%s " cLCY VERSION cLGN " (%s)",
            crash_mode ? cPIN "peruvian were-rabbit" : cYEL "afl-offset",
            use_banner);
